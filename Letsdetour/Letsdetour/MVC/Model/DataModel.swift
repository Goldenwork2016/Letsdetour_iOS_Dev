...
//        av.removeFromSuperview()
                    navigation.EndLoader(loader)
        print(response.value)   // result of response serialization
...
//            navigation.AddLoader(loader)
        ...
        Alamofire.upload(multipartFormData: { multipartFormData in
            ...
            for  (key, _) in OtherData.enumerated(){
                    multipartFormData.append(OtherData[key], withName: imageKey[key], fileName: "\(imageKey[key])/\(Int(NSDate().timeIntervalSince1970)).MOV", mimeType: "MOV")
