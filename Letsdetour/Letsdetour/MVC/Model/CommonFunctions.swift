...
extension UIImageView {
    func getImage(url : String , isUser : Bool = false)  {
        ...
        if isUser{
            if url.isValidURL{
                return self.sd_setImage(with: URL.init(string:  url), placeholderImage: #imageLiteral(resourceName: "ic_default_user"))
            }
            return self.sd_setImage(with: URL.init(string: Constants.API.IMAGE_BASE_URL + url), placeholderImage: #imageLiteral(resourceName: "ic_default_user"))

        }
        if url.isValidURL{
            return self.sd_setImage(with: URL.init(string:  url), placeholderImage: #imageLiteral(resourceName: "default_upload_image"))

        }
        return self.sd_setImage(with: URL.init(string: Constants.API.IMAGE_BASE_URL + url), placeholderImage: #imageLiteral(resourceName: "default_upload_image"))
...
class ImageController  : NSObject , UIImagePickerControllerDelegate, UINavigationControllerDelegate{
    ...
    init ( viewController : UIViewController? , sender : UIButton? , configureCellBlock : returnImageConfigureBlock?) {
        self.viewController = viewController
        self.configureCellBlock = configureCellBlock
        self.sender = sender
        super.init()
