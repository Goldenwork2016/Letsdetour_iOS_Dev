...
class AlertToast : NSObject, UIGestureRecognizerDelegate
{
    ...
    func MyToast(message:String , color : UIColor = #colorLiteral(red: 1.0, green: 1.0, blue: 1.0, alpha: 1.0) , image : UIImage = Constants.AppLogo , show : Bool = true ,  time : DispatchTime = DispatchTime.now() + 6)
    {
        if !show{
            return
        }
